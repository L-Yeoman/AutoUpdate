        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (in != null) {
                        /** 如果没有目录先建立目录 */
                        File f = new File(SDCARD_AutoUpdate_LOCAL_PATH);
                        if (!f.exists())
                            f.mkdirs();
                        /** 有目录之后建文件 */
                        File fil = new File(SDCARD_APK_LOCAL_PATH);
                        ...
                        String filePath = SDCARD_APK_LOCAL_PATH;
//
                        if(Build.VERSION.SDK_INT>=24) { //判读版本是否在7.0以上
                            File file = new File(SDCARD_APK_LOCAL_PATH);
                            //参数1 上下文, 参数2 Provider主机地址 和配置文件中保持一致   参数3  共享的文件
                            Uri apkUri =
                                    FileProvider.getUriForFile(context, "com.mapuni.autoupdate.fileprovider", file);
...
//    String apkUrl;
    public void CheckUpdate(final Context context, final String versionUrl,final int type){
